#include <sysio/sub_chain_plugin/block_s_root_processing.hpp>
#include <sysio/sub_chain_plugin/contract_s_root_object.hpp>

#include <sysio/chain/trace.hpp>


using namespace sysio;
using namespace sysio::s_root_search;

namespace sysio {
   constexpr uint32_t no_block_num = 0;
   struct block_s_root_processing_impl {
      block_s_root_processing_impl() {}
   ...
   block_s_root_processing::block_s_root_processing()
   : _my(new block_s_root_processing_impl())
   {
   }

   block_s_root_processing::~block_s_root_processing() = default;
   ...
   void block_s_root_processing_impl::signal_applied_transaction( const chain::transaction_trace_ptr& trace, const chain::packed_transaction_ptr& ptrx ) {
      ...
      if (trace->producer_block_id) {
         _head_block_id = *trace->producer_block_id;
      }
      ...
      if (!trace->producer_block_id) {
         const auto& actions = ptrx->get_transaction().actions;
         for (const auto& action : trx.actions) {
            if (isaction.account) && std::find(my->action_names.begin(), my->action_names.end(), action.name) != my->action_names.end()){
               return true;
            }
         }
   ...
   void block_s_root_processing_impl::signal_accepted_block( const chain::block_state_ptr& bsp ) {
      ...
      auto mod = [&block_id=_head_block_id,&block_timestamp=_head_block_timestamp]( contract_s_root_object& obj ) {
   ...
   void block_s_root_processing_impl::handle_rollback() {
      ...
      for (const auto& trx_iter : trxs) {
         _storage.modify( trx_iter, []( contract_s_root_object& obj ) {
            obj.forked_out = true;
         } );
      }
   }

   bool block_s_root_processing_impl::status_expiry_of_trxs(const fc::time_point& now) {
   ...
   bool block_s_root_processing_impl::ensure_storage() {
   ...
   std::optional<block_s_root_processing::trx_state> block_s_root_processing::get_trx_state( const chain::transaction_id_type& id ) const {
      ...
      return block_s_root_processing::trx_state{ .block_id = iter->block_id, .block_timestamp = iter->block_timestamp, .received = iter->received, .expiration = iter->trx_expiry, .status = status };
   }
}
